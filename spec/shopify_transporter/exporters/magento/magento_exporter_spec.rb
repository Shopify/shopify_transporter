...
RSpec.describe ShopifyTransporter::Exporters::Magento::MagentoExporter do
  context '#for' do
    ...
    it 'returns CustomerExporter for customer type' do
      expect(ShopifyTransporter::Exporters::Magento::MagentoExporter.for(type: 'customer', store_id: 1, client: fake_client))
        .to be_a(ShopifyTransporter::Exporters::Magento::CustomerExporter)
    end

    it 'returns OrderExporter for order type' do
      expect(ShopifyTransporter::Exporters::Magento::MagentoExporter.for(type: 'order', store_id: 1, client: fake_client))
        .to be_a(ShopifyTransporter::Exporters::Magento::OrderExporter)
    end

    it 'raises for any other type' do
      expect { ShopifyTransporter::Exporters::Magento::MagentoExporter.for(type: 'nonexistent_type', store_id: 1, client: fake_client) }
